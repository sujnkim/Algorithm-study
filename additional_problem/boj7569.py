...
bfs를 실행한 뒤,
조작한 data리스트의 (가장 큰 값-1) = 모두 익을 때까지 걸리는 최소 일수.

처음부터 모두 익어있는 경우를 판단하기 위해 ripen변수를 사용
값을 입력받을 때 0이 있으면 false로 저장
ripen==True이면 0을 출력, False이면 bfs를 수행한다

bfs를 실행한 수 리스트에 0이 존재하는 경우,
모두 익지 못하는 상황이므로 -1를 출력
